...
try:
    ...
    if (self.session_authenticated(session_id) and self.validate_session(session_id)):
        ...
        if (self.get_session_parameter(session_id, "questionsset")):
            logger("MAIN", "get/discovery.py", self.get_session_parameter(session_id, "username"), "NOTICE: Access Granted.")
            ...
            logger("MAIN", "get/discovery.py", self.get_session_parameter(session_id, "username"), "NOTICE: User '" + self.get_session_parameter(session_id, "username") + "' security questions not set!")
        ...
        logger("MAIN", "get/discovery.py", self.get_session_parameter(session_id, "username"), "ERROR: Access Denied.")
        
        self.execute_template("/login.html", parameters)
except:
    logger("MAIN", "get/discovery.py", "SERVER", "EXCEPTION: " + traceback.format_exc().replace('"', '&#34;').replace('<', "&#60;").replace('>', "&#62;").replace('\n', "<br>").replace(' ', "&#160;"))
